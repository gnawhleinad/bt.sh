...
#
# Creates "/tmp/bt.$BT_ID.<description checksum>.<start timestamp>" and a symlink to
# it named "/tmp/bt.$BT_ID.<description checksum>".  The former enables easily sorting
...
bt_start () {
  if [ -z "$BT_DISABLED" -o "$BT_DISABLED" = "0" ]; then
    ...
    echo "$caller $@" > "/tmp/bt.${BT_ID}.$desc_checksum.$timestamp"
    ln -s "/tmp/bt.${BT_ID}.$desc_checksum.$timestamp" "/tmp/bt.${BT_ID}.$desc_checksum" || {
...
bt_compute_cpu_sparkline () {
  ...
  local samples_per_column=$(($(wc -l "/tmp/bt.${BT_ID}.CPU" | cut -f1 -d' ') / $width))
  local nonidle_values=""
  if [ "$samples_per_column" -gt 0 ]; then
    for n in $(seq $width); do
      local samples=$(tail -n +$(($n * $samples_per_column)) "/tmp/bt.${BT_ID}.CPU" | head -n $samples_per_column)
    ...
    num_samples=$(wc -l "/tmp/bt.${BT_ID}.CPU" | cut -f1 -d' ')
    for c in $(seq $width); do
      local n=$((($c * $num_samples) / $width))
      local sample=$(tail -n +$n  "/tmp/bt.${BT_ID}.CPU" | head -n 1)
...
bt_report () {
  ...
  local total_start_ms=$(($(cat "/tmp/bt.${BT_ID}.START") / 1000000))
  local total_end_ms=$(($(cat "/tmp/bt.${BT_ID}.END") / 1000000))
  ...
  for m in $(ls -1 "/tmp/bt.${BT_ID}".*.* | sort -t '.' -k3,3 -n); do
    local m_failed=0
